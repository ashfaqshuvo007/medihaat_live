...
@section('content')
    ...
    <div class="tab-base">
        ...
        <ul class="nav nav-tabs">
            ...
            <li class="">
                <a data-toggle="tab" href="#demo-lft-tab-6" aria-expanded="false">{{ __('Advertise One') }}</a>
            </li>
        </ul>
        ...
        <div class="tab-content">
            ...
            <div id="demo-lft-tab-6" class="tab-pane fade">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <a onclick="add_slider_1()" class="btn btn-rounded btn-info pull-right">{{__('Add Advertise One Slider')}}</a>
                    </div>
                </div>
                ...
                <div class="panel">
                    <div class="panel-heading">
                        <h3 class="panel-title">{{__('Advertise One slider')}}</h3>
                    </div>
                    <div class="panel-body">
                        <table class="table table-striped table-bordered demo-dt-basic" cellspacing="0" width="100%">
                            <thead>
                            ...
                            <tbody>
                            @foreach(\App\Add::all() as $key => $add)
                                <tr>
                                    <td>{{$key+1}}</td>
                                    <td><img class="img-md" src="{{ asset($add->photo)}}" alt="Slider Image"></td>
                                    <td><label class="switch">
                                            <input onchange="update_slider_published_advertise(this)" value="{{ $add->id }}" type="checkbox" <?php if($add->published == 1) echo "checked";?> >
                                            <span class="slider round"></span></label></td>
                                    <td>
                                        <div class="btn-group dropdown">
                                            <button class="btn btn-primary dropdown-toggle dropdown-toggle-icon" data-toggle="dropdown" type="button">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-right">
                                                <li><a onclick="confirm_modal('{{route('adds.destroy', $add->id)}}');">{{__('Delete')}}</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
...
<script type="text/javascript">
    ...
    function add_slider_1(){
        $.get('{{ route('adds.create')}}', {}, function(data){
    ...
    function update_slider_published_advertise(el){
        if(el.checked){
            var status = 1;
        }
        else{
        ...
        var url = '{{ route('adds.update', 'add_id') }}';
        url = url.replace('add_id', el.value);

        $.post(url, {_token:'{{ csrf_token() }}', status:status, _method:'PATCH'}, function(data){
            if(data == 1){
                showAlert('success', 'Published add updated successfully');
            }
            else{
                showAlert('danger', 'Something went wrong');
            }
